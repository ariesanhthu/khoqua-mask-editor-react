...
export async function POST() {
  ...
  for (const name of annotators) {
    const existing = await db.prepare('SELECT id FROM users WHERE display_name = ?').get(name);
    if (!existing) {
      await db.prepare(`
  ...
  const existingProject = await db.prepare('SELECT id FROM projects LIMIT 1').get();
  ...
  const userCount = Number((await db.prepare('SELECT COUNT(*) as c FROM users').get())?.c || 0);
  const fileCount = Number((await db.prepare('SELECT COUNT(*) as c FROM dataset_files').get())?.c || 0);
